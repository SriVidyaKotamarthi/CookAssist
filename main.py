...
if __name__ == '__main__':
    ...
    st.title('Ingredients Detection and Recipe Recommendation')
    ...
    preference = ("Time", "Calories", "Rating")
    preference_index = st.sidebar.selectbox("Select Preference", range(
        len(preference)), format_func=lambda x: preference[x])
    prefdict = { "Time": "Time(mins)", "Calories": "Calories","Rating": "Rating" }
    st.session_state['ingredidents'] = ''
    if st.sidebar.button("Change Preference"):
        if 'ingredidents' in st.session_state:
            ingreds = st.session_state['ingredidents']
            print('From Session '+ ingreds)
            if len(ingreds)>0:
                obj = RecipeService("data/recipe_files/")
                obj.get_recommended_recipes(ingreds,preference[preference_index])
                obj.load_recipes_list(obj.final_dataframe)
                print(obj.recipes)
                st.write(obj.final_dataframe.to_html(escape = False), unsafe_allow_html = True)            
    ...
    if is_valid:
        ...
        if st.button('Start Detection'):
            ...
            if len(ingredidents) > 0 :
                st.session_state['Ingredients'] = ingredidents
                ...
                obj = RecipeService("data/recipe_files/")
                obj.get_recommended_recipes(ingredidents,preference[preference_index])
                obj.load_recipes_list(obj.final_dataframe)
                print(obj.recipes)
                st.write(obj.final_dataframe.to_html(escape = False), unsafe_allow_html = True)
